// Package cache provides a small Cache port for data that is derived,
// re-computable, or externally sourced — an external API response, a
// normalized/aggregated read that is expensive to recompute but never the
// sole source of truth for it. It is explicitly NOT for domain reads (the
// database is the source of truth for those), sessions (a session store
// already owns that), or anything a domain-specific cache already governs
// directly. Losing the cache directory — corruption, a wiped disk, a fresh
// deployment — must always be a non-event: every consumer recomputes or
// re-fetches on a miss, never fails because the cache is empty or
// unavailable.
//
// Two implementations: MemoryCache (in-process, for hermetic tests and as a
// caller's own boot-time fallback when the persistent backend fails to
// open) and BadgerCache (on-disk, persists across restarts — see that
...
// "<domain>:<purpose>:<id>" (e.g. "recipes:externalfind:<sha256 hex>") so
// consumers from different bounded contexts never collide on a bare key, and
// so a key's owner and purpose are obvious from a cache dump or a metrics
// label alone. This is a naming convention callers are expected to follow,
// not something the port itself validates — Set does not reject an
// unnamespaced key.
//
