...
func TestBadgerCache_RestartPreservesData(t *testing.T) {
	// "Restarting the app preserves cached external responses": open, write,
	// close, reopen at the SAME directory, confirm the value survives.
	dir := t.TempDir()
...
// for corruption-on-open handling: deleting/corrupting the cache directory
// while the app is stopped must let it start cold with no error, not fail
// boot. We simulate corruption directly (garbage bytes over badger's own
// MANIFEST file, the one badger.Open cannot proceed without) rather than
// relying on an actual crash, since that is the deterministic, reproducible
// way to exercise this path in a unit test.
...
// contract the caller's other background workers (dispatchers, schedulers)
// already rely on.
