...
// Pi-tuned Options overrides: badger's own defaults target a server with
// gigabytes of headroom (MemTableSize 64MB x NumMemtables 5 = up to 320MB
// just for memtables, plus a 256MB block cache) — this deployment's target
// is a Raspberry Pi running the WHOLE application, not badger alone, so
// every one of these is reduced well below default to keep BadgerCache's
// own resident footprint under ~100MB. The NumLevelZeroTables/Stall pair
// keeps badger's own ~1:2.5 ratio between them (matching the 5/15 default)
// rather than picking two arbitrary smaller numbers.
const (
	...
	// API responses, re-fetched at most daily by this cache's first
...
// BadgerCache is the persistent, on-disk Cache implementation, backed by
// github.com/dgraph-io/badger/v4. It is the ONLY file in this codebase
// allowed to import that package — every other consumer depends on the
// Cache port instead, enforced by a depguard rule in .golangci.yml scoped
// to this package. This keeps the dependency, and the
// corruption-recovery/GC operational concerns that come with an embedded
// LSM-tree store, fully contained here: nothing outside this file needs to
// know badger exists.
...
// the caller is expected to fall back to MemoryCache rather than fail
// startup entirely — see that type's own doc.
...
// following the same signal-cancelled-ctx shutdown pattern the caller's
// other background workers use.
