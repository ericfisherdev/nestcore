...
// TOTP), mirroring the qrcode and crypto packages: it isolates the
// third-party dependency behind a small, stateless surface so the caller's
// application layer depends on its own minimal interface rather than the
// library directly, keeping that layer's tests hermetic and the library
// swappable.
package totp
...
// enforce a durable replay guard across steps (the same code must never be
// accepted twice, even across the skew window, which requires knowing WHICH
// of the up to three candidate steps a submitted code actually corresponds
// to).
//
