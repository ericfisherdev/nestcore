...
func TestGenerateSecret_ReturnsUsableSecretAndOTPAuthURL(t *testing.T) {
	p := totp.NewProvider()
	secret, otpauthURL, err := p.GenerateSecret("ExampleApp", "alice")
	if err != nil {
	...
	if key.Issuer() != "ExampleApp" {
		t.Errorf("otpauth URL issuer = %q, want ExampleApp", key.Issuer())
	}
...
func TestGenerateSecret_EachCallProducesADifferentSecret(t *testing.T) {
	p := totp.NewProvider()
	secret1, _, err := p.GenerateSecret("ExampleApp", "alice")
	if err != nil {
		t.Fatalf("GenerateSecret (1): %v", err)
	}
	secret2, _, err := p.GenerateSecret("ExampleApp", "alice")
	if err != nil {
...
func TestValidate_AcceptsCurrentCodeRejectsWrongCode(t *testing.T) {
	p := totp.NewProvider()
	secret, _, err := p.GenerateSecret("ExampleApp", "alice")
	if err != nil {
		t.Fatalf("GenerateSecret: %v", err)
	}
...
func TestValidate_RejectsCodeForADifferentSecret(t *testing.T) {
	p := totp.NewProvider()
	secretA, _, err := p.GenerateSecret("ExampleApp", "alice")
	if err != nil {
		t.Fatalf("GenerateSecret (A): %v", err)
	}
	secretB, _, err := p.GenerateSecret("ExampleApp", "bob")
	if err != nil {
