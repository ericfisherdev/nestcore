...
// fakeWebAuthnCredentialRepo is an in-memory domain.WebAuthnCredentialRepository.
type fakeWebAuthnCredentialRepo struct {
	byMember map[domain.MemberID][]domain.WebAuthnCredential
}
...
func (f *fakeWebAuthnCredentialRepo) Create(_ context.Context, householdID domain.HouseholdID, cred *domain.WebAuthnCredential) error {
	cp := *cred
	...
	for memberID, creds := range f.byMember {
		for _, c := range creds {
			if bytes.Equal(c.UserHandle, handle) {
				out := make([]domain.WebAuthnCredential, len(creds))
				copy(out, creds)
...
var _ domain.WebAuthnCredentialRepository = (*fakeWebAuthnCredentialRepo)(nil)
...
// webauthnSpecTestVectorNoneES256's clientDataJSON exactly — the W3C
// test vector is only valid against this specific RP configuration.
const (
...
// newWebAuthnServiceFixture wires a WebAuthnService with fully
// controllable fakes, so tests can both exercise the service and assert
// directly against the fake repository's state.
...
// webauthnSpecTestVectorNoneES256 returns the W3C WebAuthn spec's
// published "none" attestation, ES256 registration test vector
// (https://www.w3.org/TR/webauthn-3/#sctn-test-vectors-none-es256), in
// the same JSON shape a browser's PublicKeyCredential.toJSON() produces
// — a real, valid, deterministic registration ceremony response, so
// WebAuthnService.FinishRegistration's success path can be tested
// without a live authenticator or hand-rolled attestation signing. It is
// only valid against testWebAuthnRPID/testWebAuthnRPOrigin (encoded in
// its clientDataJSON) and its own fixed challenge (returned here).
...
// validSessionFor builds the webauthn.SessionData BeginRegistration
// would have produced for the spec test vector's challenge and
// memberID's derived handle — CredParams must list ES256
// (webauthncose.AlgES256) to match the test vector's actual public key
// algorithm, or CreateCredential's attestation verification rejects it
// regardless of the challenge/user id matching.
...
// ---------------------------------------------------------------------------
// Synthetic login assertions
//
// webauthnSpecTestVectorNoneES256 (above) is fixed at authenticator
// counter 0 — the W3C spec's own published assertion test vector for
// that credential — so it can prove FinishLogin's happy path but cannot
// exercise sign-count comparison at OTHER counter values (the counter is
// part of authenticatorData, which is itself covered by the signature —
// changing it without re-signing would just make verification fail, not
// simulate a different real assertion). syntheticAuthenticator generates
// a real ES256 keypair and signs a real, spec-shaped assertion response
// for ANY chosen counter, RP ID, origin, and challenge, so FinishLogin's
// sign-count handling can be exercised through its REAL cryptographic
// verification path rather than a shortcut around it.
// ---------------------------------------------------------------------------

// syntheticAuthenticator is a minimal, in-test ES256 "authenticator"
// that can sign a WebAuthn assertion for one credential at any given
// counter value.
...
// alg=ES256, crv=P-256) go-webauthn's own attestation object encodes —
// the same field layout webauthnSpecTestVectorNoneES256's fixed
// credentialPubKeyHex uses (a5 01 02 03 26 20 01 21 58 20 <x32> 22 58 20
// <y32>), built here from the freshly generated key's own X/Y
// coordinates (read via PublicKey.Bytes' SEC 1 uncompressed-point
// encoding — 0x04 || X || Y — rather than the deprecated PublicKey.X/
// PublicKey.Y fields directly) so toWebAuthnCredential-style stored
// credentials round-trip correctly through webauthncose.ParsePublicKey.
func (a *syntheticAuthenticator) cosePublicKey() []byte {
	uncompressed, err := a.priv.PublicKey.Bytes()
	if err != nil {
		// Bytes() can only fail for an invalid key or an unsupported
		// curve; a freshly generated P-256 key is neither.
...
func TestWebAuthnService_FinishRegistration_MismatchedUserHandle_Fails(t *testing.T) {
	...
	// session bound to a DIFFERENT member's handle than the one
	// FinishRegistration is called for must fail, not silently register
	// the credential under the wrong identity.
	t.Parallel()
	svc, repo, _ := newWebAuthnServiceFixture(t)
	memberID := domain.NewMemberID()
...
func TestWebAuthnService_BeginLogin_ReturnsEmptyAllowCredentials(t *testing.T) {
	...
	// AC: usernameless login — empty allowCredentials means the browser
	// must be free to offer ANY of its discoverable credentials for this
	// RP, not a specific member's.
...
// ---------------------------------------------------------------------------
// Sign-count handling — exercised with syntheticAuthenticator (real
// ES256 signatures at counter values the fixed W3C vector cannot
// express) through FinishLogin's real cryptographic verification path.
// ---------------------------------------------------------------------------

func TestWebAuthnService_FinishLogin_SignCountIncreases(t *testing.T) {
	t.Parallel()
	svc, repo, handles := newWebAuthnServiceFixture(t)
	memberID := domain.NewMemberID()
	householdID := domain.NewHouseholdID()
...
func TestWebAuthnService_FinishLogin_SignCountDecreases_StillAllowsLogin(t *testing.T) {
	t.Parallel()
	svc, repo, handles := newWebAuthnServiceFixture(t)
	memberID := domain.NewMemberID()
	householdID := domain.NewHouseholdID()
...
// testWebAuthnCredentialForLogin builds a stored domain.WebAuthnCredential
// shaped for a login test. publicKey is normally a
// syntheticAuthenticator's own cosePublicKey().
func testWebAuthnCredentialForLogin(memberID domain.MemberID, credentialID, userHandle, publicKey []byte, signCount uint32) *domain.WebAuthnCredential {
