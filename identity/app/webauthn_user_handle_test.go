...
// TestWebAuthnUserHandleDeriver_Derive_IsDeterministic covers
// determinism both within one deriver instance (two calls) AND across
// two SEPARATE instances constructed from the same key — the latter is
// what actually proves the derivation is a pure function of (key,
// memberID) with no hidden per-instance state, since a stateful
// implementation could still pass a same-instance-only check while
// producing a different handle after a process restart (a fresh
// instance).
...
func TestWebAuthnUserHandleDeriver_Derive_DistinctPerMember(t *testing.T) {
	...
	handleA := d.Derive(domain.NewMemberID())
	handleB := d.Derive(domain.NewMemberID())
...
func TestWebAuthnUserHandleDeriver_Derive_WithinSpecLength(t *testing.T) {
	// WebAuthnID's own doc: "an opaque byte sequence with a maximum
	// size of 64 bytes".
	t.Parallel()
	d, err := app.NewWebAuthnUserHandleDeriver([]byte("a-test-webauthn-user-handle-key"))
	if err != nil {
	...
	handle := d.Derive(domain.NewMemberID())
...
func TestNewWebAuthnUserHandleDeriverFromSecret_DerivesDistinctKeysPerPurpose(t *testing.T) {
	...
	d1, err := app.NewWebAuthnUserHandleDeriverFromSecret(secret, "nestcore:identity:webauthn-user-handle:v1")
	if err != nil {
