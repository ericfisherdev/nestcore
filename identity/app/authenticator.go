...
// passwordHasher is the minimal seam over argon2id password hashing
// (ISP): the identity app services need only to derive and check hashes,
// not to know the cost parameters or the PHC encoding. Satisfied by
// *crypto.Hasher (github.com/ericfisherdev/nestcore/crypto).
//
// It exists so tests can inject a cheap-cost hasher. Hashing at the
// production cost dominates a test suite's runtime; because the cost is
// recorded in the PHC string and read back by Verify, a cheaply-hashed
// fixture verifies through exactly the same code path as a production
// hash.
...
type Authenticator struct {
	...
	// dummyHash normalizes Login timing when the email is unknown:
	// verifying against it makes the "no such user" path cost about as
	// much as the "wrong password" path, preventing user enumeration via
...
func New(repo domain.CredentialRepository, hasher passwordHasher) *Authenticator {
	...
	if err != nil {
		// Hash only fails if the system RNG is unavailable; that is fatal
...
func (a *Authenticator) Login(ctx context.Context, email, password string) (domain.MemberID, error) {
	...
	if err != nil {
		if errors.Is(err, domain.ErrInvalidCredentials) {
			...
			// about the same as the wrong-password path, preventing
			// timing-based enumeration. Then return the generic sentinel.
			_, _ = a.hasher.Verify(password, a.dummyHash)
			return domain.MemberID{}, domain.ErrInvalidCredentials
		}
		// A genuine lookup failure (e.g. database outage) is surfaced
		// rather than masked as invalid credentials, so the caller can
		// distinguish it from a real 401.
		return domain.MemberID{}, fmt.Errorf("authenticate: %w", err)
	}
