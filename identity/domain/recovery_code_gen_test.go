...
// recoveryCodeSymbolAlphabet mirrors domain's unexported
// recoveryCodeAlphabet for black-box assertions (Crockford's Base32, no
...
func TestGenerateRecoveryCode_FormatIsFourDashSeparatedGroupsOfFour(t *testing.T) {
	...
	// 16 Base32 symbols (80 bits / 5 bits-per-symbol) + 3 dashes = 19
	// chars. This is the direct regression check for the entropy bug: a
	// byte-per-symbol scheme would produce only 10 symbols (50 bits)
	// here, not 16 (80 bits).
	if len(code) != 19 {
...
func TestNormalizeRecoveryCode_RoundTripsCaseAndDashVariants(t *testing.T) {
	...
	for _, v := range variants {
		got := domain.NormalizeRecoveryCode(v)
		if got != want {
...
func TestNormalizeRecoveryCode_FoldsVisuallyConfusableCharacters(t *testing.T) {
	...
	for input, want := range tests {
		if got := domain.NormalizeRecoveryCode(input); got != want {
