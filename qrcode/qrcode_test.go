...
func TestPNGDataURI_ReturnsDecodablePNG(t *testing.T) {
	uri, err := qrcode.PNGDataURI("https://example.ts.net/go/claim-task/abc-123?exp=1&sig=xyz", 8)
	if err != nil {
...
func TestPNGDataURI_DifferentContentProducesDifferentImages(t *testing.T) {
	a, err := qrcode.PNGDataURI("https://example.ts.net/go/claim-task/1", 8)
	if err != nil {
		t.Fatalf("PNGDataURI(a): %v", err)
	}
	b, err := qrcode.PNGDataURI("https://example.ts.net/go/claim-task/2", 8)
	if err != nil {
...
func TestPNGDataURI_RejectsInvalidModuleSize(t *testing.T) {
	...
	for _, tt := range tests {
		t.Run(tt.name, func(t *testing.T) {
			_, err := qrcode.PNGDataURI("https://example.ts.net/go/claim-task/1", tt.moduleSize)
...
func TestPNGDataURI_AcceptsBoundaryModuleSizes(t *testing.T) {
	...
	for _, size := range []int{1, 50} {
		if _, err := qrcode.PNGDataURI("https://example.ts.net/go/claim-task/1", size); err != nil {
