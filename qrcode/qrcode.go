...
// mirroring the render package: it renders a QR code entirely server-side
// and returns it as a self-contained "data:image/png;base64,..." URI, so
// callers (Templ components) embed it directly in an <img src> with no
// client-side QR JS and no extra HTTP round trip to fetch the image.
//
// The library used is github.com/yeqown/go-qrcode/v2 — an earlier
// candidate, skip2/go-qrcode, was rejected because that module is
// unmaintained (last release predates Go modules' current tooling
// expectations, no open-source activity in years); yeqown/go-qrcode is
// actively maintained and was chosen instead.
package qrcode
