...
// DateTime in IFD0 — the builder below generalizes to both IFDs so each
// test only has to name the tags/values it cares about.
...
// entries are only supported in exifSubFields, which is all these tests
// need (DateTimeOriginal/DateTime/DateTimeDigitized).
...
// exifJPEGFixture splices a synthetic EXIF APP1 segment (built from
// ifd0Fields/exifSubFields) onto a real, decodable JPEG's own segments
// (base's own SOI is dropped in favor of the fixture's), so the result is
// both a genuinely valid, fully decodable photo AND carries the exact EXIF
// tags a test names.
...
// TestTakenAtAndOrientationIgnoresDateTimeDigitizedWithoutOriginal cover the
// provenance tightening: a plain DateTime or DateTimeDigitized tag, with NO
// DateTimeOriginal present, must NOT be accepted as a capture time — those
// tags don't guarantee the photo was just taken with a camera (an
// edited/re-saved/scanned image can carry either without ever having
// DateTimeOriginal set), which defeats a freshness gate's whole purpose.
...
// "no GPS EXIF survives": Scrub with orientation 1 (already upright) or 0
// (unknown) removes the ENTIRE EXIF APP1 segment at the byte level —
...
// TestScrubStripsExifPrecededByFillBytes covers the fill-byte handling: the
// JPEG spec allows extra 0xFF padding bytes immediately before a marker's
// code byte outside scan data, and stripJPEGExif must consume them and
// still find/remove the EXIF APP1 segment that follows.
...
// TestScrubNonJPEGReturnsUnchanged covers Scrub's documented no-op for
// input that is not a JPEG at all (no SOI marker) — a caller only invokes
// Scrub after sniffing a JPEG prefix, but Scrub itself stays safe if ever
// called otherwise.
...
// 8x8-pixel-aligned solid-color blocks (aligned to JPEG's minimum coded
// unit so each block's interior survives lossy compression essentially
// exactly), arranged cols x rows, so a geometric transform's effect on
// block positions can be checked by sampling each block's center pixel.
...
// blockCenterColor decodes data and returns the pixel at the center of
// block (col, row) in a cols x rows, 8px-block grid — used to check a
// transform's effect without being thrown off by lossy edge blur.
...
func TestScrubReencodesAndBakesOrientation(t *testing.T) {
	...
	t.Run("orientation 6 rotates 90 CW and swaps dimensions", func(t *testing.T) {
		...
		// to the top-right; bottom-left (blue, block 0,1) moves to
		// top-left.
		if !closeEnough(blockCenterColor(t, got, 0, 0), blue) {
...
// without needing DQT/DHT/SOS or any actual entropy-coded scan data. This
// lets a test claim an enormous pixel count in a tiny file, exactly the
// decompression-bomb shape maxDecodePixels guards against, without needing
// to construct gigabytes of real (or even valid) scan data.
...
// TestScrubReencodeBoundsPixelsBeforeFullDecode covers the decompression-
// bomb guard: Scrub's orientation-bake path (Scrub runs BEFORE
// PhotoStore.Put ever validates anything) must reject a claimed pixel
// count over maxDecodePixels using DecodeConfig alone, the same way
// LocalPhotoStore.Put already does, rather than attempting the full,
// memory-heavy image.Decode first.
...
// TestScrubRejectsMalformedJPEGStructure covers the privacy fix: a JPEG
// whose internal segment structure is truncated or malformed is REJECTED
// with domain.ErrInvalidPhoto — never panics, and never falls back to
// passing the unexamined remainder of the bytes through unscrubbed.
...
// TestScrubRejectsJunkByteEvenWhenValidExifFollows is the exact regression
// fixture for the byte-level strip's privacy-safe default: SOI + one stray
// junk byte (not a marker) + a VALID EXIF APP1 segment + the rest of a
// valid JPEG must be rejected outright, never copied through with the
// still-intact EXIF segment silently surviving.
...
// TestScrubReencodeInvalidImageMapsToErrInvalidPhoto covers the
// error-mapping contract: a DecodeConfig or Decode failure in the
// orientation-bake path must wrap domain.ErrInvalidPhoto, not a bare error.
