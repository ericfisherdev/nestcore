...
// uses, mirroring the layout goexif2's own (*Exif).DateTime uses
// internally.
const exifDateLayout = "2006:01:02 15:04:05"

// reencodeQuality is the JPEG quality used when Scrub must fully re-encode
// a photo to bake in its EXIF Orientation (see Scrub's doc). 90 keeps
// visible quality loss minimal while still compressing well below the
// original — re-encoding only ever runs for the minority of uploads whose
// Orientation is not already 1 (upright)/0 (unknown).
const reencodeQuality = 90

// TakenAtAndOrientation returns the EXIF capture time (UTC) and the
// Orientation tag from a JPEG's already-buffered bytes, or (nil, 0) when
// the bytes carry no decodable EXIF at all. Hardened against a crafted
// image the same way TakenAt is: a panic deep in the third-party parser on
// malformed input is recovered, never propagated.
//
// Capture time comes from DateTimeOriginal ONLY — no DateTime/
// DateTimeDigitized fallback, unlike TakenAt (which prefers
// DateTimeOriginal but falls back to DateTime): a caller using this
// stricter variant typically has a freshness/provenance requirement (e.g.
// gating that a photo was just taken with a camera) that a DateTime
// fallback would undermine.
//
// No EXIF OffsetTimeOriginal (tag 0x9011) handling is attempted: goexif2
// v1.1.0's internal tag table only loads tag ids it explicitly maps, so
// OffsetTimeOriginal is unreachable through the library's public API. A
// naive (offset-less) EXIF timestamp is therefore interpreted in the
// server's own local time zone — exactly what goexif2's own DateTime()
// already falls back to (it also honors a Canon MakerNote timezone field
// when present, which this function keeps by reusing the same tag lookup
// DateTime() performs). A caller relying on this must be a single-timezone
// deployment where the server's own local timezone IS the correct
// interpretation.
...
// dateTimeOriginal returns the EXIF DateTimeOriginal tag, normalized to
// UTC, or nil when absent or unparseable — see TakenAtAndOrientation's doc
// for why this is the ONLY tag consulted (no DateTime/DateTimeDigitized
// fallback).
...
//
// Deliberate, documented tradeoff: goexif2 is read-only, and a surgical
// "remove only the GPS IFD, keep everything else" rewrite would mean
// hand-editing a TIFF directory structure in place (patching an IFD's
...
// software version, etc.) on a photo whose privacy motivation is not
// limited to GPS. Stripping the WHOLE EXIF APP1 segment is simpler,
// strictly more private, and is what this function does for the common
// case (Orientation already 1 or 0): stripJPEGExif below removes the
// "Exif\0\0"-signed APP1 segment at the byte level, leaving every other
// JPEG segment (and the entropy-coded scan data) untouched — no
...
// correct), Scrub instead fully decodes the image and re-encodes it with
// the orientation baked into the pixel data (reencodeUpright below) —
// heavier (one JPEG re-compression pass, at reencodeQuality) but only for
// the minority of uploads that actually need rotating, and it also strips
...
// reencodeUpright decodes data, applies the EXIF Orientation transform o
// (2-8) to the pixel data, and re-encodes as a fresh JPEG with no EXIF at
// all.
//
// Bounds the claimed pixel count via DecodeConfig BEFORE the full Decode
// below, which allocates a full pixel buffer sized to it — a
// decompression bomb (a small file whose header claims enormous
// dimensions) would otherwise let this allocation run unchecked. This
// mirrors validateAndStage's identical maxDecodePixels guard verbatim,
// including the constant — Scrub runs BEFORE Put ever sees the bytes in a
// caller's chore-proof-style upload pipeline, so Put's own guard cannot be
// relied on to catch this first.
...
// applyOrientation returns a new RGBA image with src reoriented according
// to the EXIF Orientation tag o (2-8; callers only invoke this for o !=
// 1/0). The transforms are defined by the EXIF/TIFF spec (Exif 2.3 section
// 4.6.4, tag 0x0112):
//
...
// from "is a JPEG but its internal segment structure is malformed", which
// is what everything below actually guards against.
//
...
// data verbatim — a structurally-broken segment BEFORE a real EXIF APP1
// segment could otherwise make the walk bail out early and copy everything
// after it, including the still-intact EXIF, straight into the "scrubbed"
// output. Rejecting instead is the privacy-safe default.
//
