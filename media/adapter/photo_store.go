...
// MEDIA_ROOT/households/<household>/<class>/<aa>/<hash>.<ext> (see
// classKeyPrefix), so identical bytes uploaded for the same
// domain.PhotoClass de-duplicate on disk, a ref never collides across
// households, and — the reason the class segment exists — bytes uploaded
// under one class can never collide with, or be resolved as, another
// class's bytes even if the content happens to be byte-identical.
//
...
// unregistered class — removes the staging file, leaving no partial upload
// behind.
func (s *LocalPhotoStore) Put(_ context.Context, householdID identity.HouseholdID, class domain.PhotoClass, r io.Reader) (domain.PutResult, error) {
	...
	// Content-addressed, so a file already at full is byte-identical;
	// Rename is atomic and harmlessly replaces it.
...
// unknown. The returned *os.File natively satisfies domain.PhotoReader
// (Read, ReadAt, Seek, Close), which lets EXIF extraction read directly
// from disk instead of first buffering the file into memory.
...
// Delete removes a stored photo; a missing file is not an error
// (idempotent).
...
// unknown; ttl is ignored (see the domain.PhotoStore.URL doc for why a
// local backend cannot honestly return a browser-navigable URL from ref
// alone).
