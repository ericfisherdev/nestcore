...
// testStoreClass is a PhotoClass registered once for this test package's
// use — registration is process-global (see domain.RegisterPhotoClass), so
// a single package-level var, not one per test, is what a real consumer
// would do too.
var testStoreClass = domain.RegisterPhotoClass("test_store_class")
...
func TestPutStoresAndOpensAndDeletes(t *testing.T) {
	...
	result, err := s.Put(context.Background(), hh, testStoreClass, bytes.NewReader(want))
	if err != nil {
		t.Fatalf("Put: %v", err)
	}
	if result.Ref == "" {
...
func TestPutContentAddressedDeduplicates(t *testing.T) {
	...
	hh := identity.NewHouseholdID()
	data := jpegBytes(t)
	r1, err := s.Put(context.Background(), hh, testStoreClass, bytes.NewReader(data))
	if err != nil {
		t.Fatalf("first Put: %v", err)
	}
	r2, err := s.Put(context.Background(), hh, testStoreClass, bytes.NewReader(data))
	if err != nil {
...
func TestPutRejections(t *testing.T) {
	...
	for _, tc := range cases {
		t.Run(tc.name, func(t *testing.T) {
			if _, err := tc.store.Put(context.Background(), hh, testStoreClass, bytes.NewReader(tc.data)); !errors.Is(err, tc.wantErr) {
...
// TestPutRejectionLeavesNoPartialFile covers a rejected (oversize) upload
// must not leave a partial file behind in the store root.
func TestPutRejectionLeavesNoPartialFile(t *testing.T) {
	...
	hh := identity.NewHouseholdID()
	if _, err := s.Put(context.Background(), hh, testStoreClass, bytes.NewReader(pngBytes(t))); !errors.Is(err, domain.ErrPhotoTooLarge) {
...
// TestPutStreamsWithoutBufferingWholeFile is a direct behavioral proof that
// Put reads in bounded chunks (io.Copy's default buffer) rather than one
// call sized to the whole upload.
func TestPutStreamsWithoutBufferingWholeFile(t *testing.T) {
	...
	if _, err := s.Put(context.Background(), hh, testStoreClass, tracker); err != nil {
		t.Fatalf("Put: %v", err)
	}
...
// TestPutNamespacesKeyByClass covers the core class-namespacing contract:
// identical bytes uploaded under different domain.PhotoClass values must
// land under different storage prefixes, so one registered class's bytes
// can never collide with, or be reinterpreted as, another's even when the
// content is byte-identical.
func TestPutNamespacesKeyByClass(t *testing.T) {
	classA := domain.RegisterPhotoClass("test_namespace_class_a")
	classB := domain.RegisterPhotoClass("test_namespace_class_b")
	s := newStore(t, 10<<20)
	hh := identity.NewHouseholdID()
	data := jpegBytes(t)

	a, err := s.Put(context.Background(), hh, classA, bytes.NewReader(data))
	if err != nil {
		t.Fatalf("Put(classA): %v", err)
	}
	b, err := s.Put(context.Background(), hh, classB, bytes.NewReader(data))
	if err != nil {
		t.Fatalf("Put(classB): %v", err)
	}

	if a.Ref == b.Ref {
		t.Fatalf("identical bytes under different classes must not share a ref: a=%s b=%s", a.Ref, b.Ref)
	}
	// The content hash itself is class-independent (it is a property of the
	// bytes, not of where they are namespaced) — only the ref's prefix
	// differs.
	if a.ContentHash != b.ContentHash {
		t.Fatalf("identical bytes produced different content hashes across classes: a=%s b=%s", a.ContentHash, b.ContentHash)
	}
	...
	for _, tc := range []struct {
		...
		{"class a", a.Ref.String(), "test_namespace_class_a"},
		{"class b", b.Ref.String(), "test_namespace_class_b"},
	} {
		t.Run(tc.name, func(t *testing.T) {
...
// TestPutRejectsUnregisteredClass covers Put's fail-fast contract for a
// class this deployment never registered: it must reject before writing
// anything, leaving no partial file behind.
func TestPutRejectsUnregisteredClass(t *testing.T) {
	root := t.TempDir()
	...
	// The zero value must be rejected too, so a caller that forgot to
	// choose a class can never default into some already-registered
	// namespace.
	var zero domain.PhotoClass
	if _, err := s.Put(context.Background(), hh, zero, bytes.NewReader(jpegBytes(t))); err == nil {
		t.Fatal("Put with the zero-value PhotoClass must fail")
	}
...
// TestOpenAndDeleteServeLegacyRef covers backward compatibility: a photo
// stored under a bare <household>/<aa>/<hash>.<ext> ref, with no
// "households/" or class prefix, must remain servable — LocalPhotoStore
// treats ref as an opaque relative path in Open/Delete and never assumes
// its shape.
...
func TestURL(t *testing.T) {
	...
	stored, err := s.Put(context.Background(), hh, testStoreClass, bytes.NewReader(jpegBytes(t)))
	if err != nil {
		t.Fatalf("Put: %v", err)
	}
