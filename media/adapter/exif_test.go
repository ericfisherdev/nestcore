...
// exifJPEG builds a minimal JPEG (SOI + APP1/Exif + EOI) carrying a single
// IFD0 DateTime tag (0x0132), so the EXIF reader has a real capture date to
// decode. datetime must be the 19-char "YYYY:MM:DD HH:MM:SS" EXIF form.
...
func TestExifTakenAtPresent(t *testing.T) {
	...
	// goexif parses the zoneless EXIF time in time.Local; the adapter
	// normalizes to UTC, so the expectation is the same local instant
	// converted to UTC.
