...
// S3Params configures NewS3PhotoStore. It mirrors an application's own S3
// config field-for-field but is its own type: this adapter depends on
// configuration only through the composition root passing plain values in,
// never by importing any application's own config package directly (DIP).
...
// endpoint on the LAN, or Cloudflare R2. Photos use the identical
...
// the validated file. Open buffers the complete object into memory, bounded
// by MaxUploadBytes (a GetObject response body is sequential-read only, but
// domain.PhotoReader needs ReadAt/Seek for EXIF extraction, and every
// genuinely-stored photo is already capped at MaxUploadBytes by Put).
...
type S3PhotoStore struct {
	...
	// conditional, not unconditional: verified against a real MinIO
	// instance (no KMS configured), MinIO does NOT silently ignore/no-op
	// an SSE-S3 request — it rejects the PutObject outright with 501
	...
	// (no custom Endpoint); a custom endpoint (MinIO/Garage/R2) never gets
	// the header at all, which is also simply correct for R2/Garage, whose
	// own encryption models don't take an S3-protocol SSE header either.
	requestSSE bool
}
...
// rather than surfacing as an opaque error on a household's first photo
// upload.
...
// (AES256) only against real AWS S3 (see the requestSSE field doc).
func (s *S3PhotoStore) Put(ctx context.Context, householdID identity.HouseholdID, class domain.PhotoClass, r io.Reader) (domain.PutResult, error) {
...
// presigned GET URL valid for ttl, or s.presignTTL when ttl is
// non-positive.
...
// every household (domain.ObjectLister; a storage reaper's source of
// truth). The key layout (buildStorageKey: households/<household>/<class>/
// ...) puts <household> BEFORE <class>, so no single S3 ListObjectsV2
...
// (see keyBelongsToClass). For a single-household appliance's expected
// scale (not a multi-tenant SaaS bucket with millions of objects), a
// full-tree scan per reaper pass is an acceptable cost.
...
// verbatim) without downloading it (domain.ObjectExister) — a storage
// migrator's idempotency check before uploading a content-addressed object
// a different row's migration may have already written at the same key
// (see PutAt's doc).
...
// not Put, is what a storage migrator calls). Mirrors Put's own
// SSE-S3/Cache-Control handling exactly, so an object the migrator writes
// is indistinguishable from one a normal upload would have produced at the
// same key.
...
// keyBelongsToClass reports whether key's class path segment
// ("households/<household>/<class>/...") equals classPrefix — see
