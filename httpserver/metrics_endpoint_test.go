...
// testNamespace is the metrics namespace used across this file's tests.
const testNamespace = "testapp"

// newMetricsServer builds a server with the full metrics wiring: a fresh
// registry, the HTTP request metrics, and the promhttp scrape handler,
// mirroring a composition root's own wiring.
func newMetricsServer(t *testing.T) *http.Server {
	t.Helper()
	reg := metrics.NewRegistry()
	return httpserver.New(testConfig(), httpserver.Deps{
		Logger:         slog.New(slog.NewTextHandler(io.Discard, nil)),
		HTTPMetrics:    metrics.NewHTTPMetrics(reg, testNamespace),
...
func TestMetricsEndpoint(t *testing.T) {
	...
	wantSample := testNamespace + `_http_requests_total{method="GET",route="GET /healthz",status="200"} 1`
