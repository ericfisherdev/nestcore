...
func TestNewDerivesConnectionTimeoutsFromRequestTimeout(t *testing.T) {
	cfg := Config{Server: config.ServerConfig{Addr: ":0", RequestTimeout: 45 * time.Second}}
...
// TestZeroRequestTimeoutOmitsPerRequestDeadline covers the guard against a
// hand-built ServerConfig whose RequestTimeout was never set (i.e. it never
// went through config.ServerConfig.Validate, which enforces a floor above
// requestTimeoutMargin): New must not install an already-expired per-request
// context on every request. The handler should observe no deadline at all,
// not one that expired before the request began.
func TestZeroRequestTimeoutOmitsPerRequestDeadline(t *testing.T) {
	cfg := Config{Server: config.ServerConfig{Addr: ":0"}} // RequestTimeout left at zero
	...
	if hasDeadline {
		t.Error("request context has a deadline with RequestTimeout=0; Timeout middleware should have been omitted")
	}
}
